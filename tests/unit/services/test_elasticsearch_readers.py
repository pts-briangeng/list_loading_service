...
class TestElasticSearchFileReaders(unittest.TestCase):
    ...
    def test_reads_csv_file_correctly(self, mock_open, mock_csv_reader, mock_os_stat):
        mock_open.return_value = mock.MagicMock(spec=file)
        mock_csv_reader.return_value = mocks.generator(['account_no'])
        mock_os_stat.return_value = mock.MagicMock()
        mock_os_stat.return_value.st_size = 1
    ...
    @mock.patch.object(openpyxl, 'load_workbook', autospec=True)
    @mock.patch.object(readers, 'open', create=True)
    def test_get_correct_reader_based_on_extension(self, mock_open, mock_load_workbook):
        mock_open.return_value = mock.MagicMock(spec=file)
